...
const OUTCOME_LABELS: Record<string, { text: string; color: string }> = {
  CONTAINED: { text: "해결됨", color: "text-[var(--success-green)]" },
  ESCALATED: { text: "악화됨", color: "text-[var(--hp-red)]" },
  EXPIRED: { text: "시효만료", color: "text-[var(--text-secondary)]" },
};
...
function GaugeBar({
  ...
  return (
    <div className="flex items-center gap-1.5 text-[10px]">
      ...
      <span className="w-7 text-right text-[var(--text-secondary)]">{value}%</span>
    </div>
  );
}
